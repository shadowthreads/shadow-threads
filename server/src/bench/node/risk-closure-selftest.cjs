...
function makeInput() {
  return {
    ...
    proposedDelta: {
      ...
      base: { revisionHash: 'risk-selftest-base' },
      target: { revisionHash: 'risk-selftest-target' },
...
function buildContract(modules) {
  ...
  return modules.buildClosureContractV1({
    proposedDelta: input.proposedDelta,
...
function main() {
  try {
    ...
    const contractA = buildContract(modules);
    const contractB = buildContract(modules);
    const hasL3 = contractA.rejected.some((entry) => entry && entry.riskLevel === 'L3');
    const hasPromotion = contractA.suggestions.some((entry) => entry && entry.actionType === 'PROMOTE_TO_L3_REVIEW');
    const deterministic = modules.stableStringify(contractA) === modules.stableStringify(contractB);
    const ok = deterministic && (!hasL3 || hasPromotion);

    process.stdout.write(ok ? 'RISK_CLOSURE_SELFTEST_OK\n' : 'RISK_CLOSURE_SELFTEST_FAIL\n');
    process.exit(ok ? 0 : 1);
  } catch (_error) {
